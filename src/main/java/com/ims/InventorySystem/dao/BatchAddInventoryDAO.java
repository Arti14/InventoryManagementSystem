 ...
 *
 */
public class BatchAddInventoryDAO {

	public static final String SQS_QUEUE_URL = 
			"https://sqs.ap-southeast-1.amazonaws.com/074379253665/INVENTORY_BATCH_ADD";
	
	private S3Client s3;
	private SQSClient sqs;
	
	public BatchAddInventoryDAO(final AWSCredentials credentials) {
		s3 = new S3Client(credentials);
		sqs = new SQSClient(credentials);
	}
	
	public void queueBatchAddInventoryRequest(BatchAddInventoryRequest request) 
			throws IOException {
		sqs.sendMessage(SQS_QUEUE_URL, request.serialize());
	}
	
	public BatchAddInventoryRequest fetchBatchAddInventoryRequest() 
			throws IOException {
	...
	public void deleteFromQueueBatchAddInventoryRequest(
			BatchAddInventoryRequest request) {
		sqs.deleteMessage(SQS_QUEUE_URL, request.getReceiptHandle());
	}
	
	public InputStream getBatchAddInventoryFileStream(
			BatchAddInventoryRequest request) {
